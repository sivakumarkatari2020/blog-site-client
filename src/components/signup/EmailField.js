...
export const EmailField = (props) => {
    ...
    const validateMail = (mail) => {
        if((/[^@ \t\r\n]+@[^@ \t\r\n]+\.[^@ \t\r\n]+/.test(mail) && mail.length >=4) || mail.length <= 0 ){
            setValues({...values,isMailCorrect: true,mail: mail.trim(),isValidated: true});
        }
        else{
            setValues({...values,isMailCorrect: false,mail: mail.trim(),isValidated: true});
        }
    }
